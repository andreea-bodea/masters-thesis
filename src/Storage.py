...
def check_document_exists(file_hash):
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("SELECT * FROM documents_pdf WHERE file_hash = %s", (file_hash,))
        return cur.fetchone()
    
def insert_record(file_name, file_hash, pdf_bytes, text_with_pii, text_pii_deleted, text_pii_labeled, text_pii_synthetic, details):
