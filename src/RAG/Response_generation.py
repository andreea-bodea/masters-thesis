...
def get_all_responses(table_responses_name, index_name, file_name, database_file):
    ...
    for question in questions:  # FOR EACH QUESTION: UTILITY & PRIVACY (UNTARGETED ATTACK)
        ...
        response_diffractor1, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_diffractor1"])
        response_diffractor2, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_diffractor2"])
        response_diffractor3, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_diffractor3"])
        response_dp_prompt1, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dp_prompt1"])
        response_dp_prompt2, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dp_prompt2"])
        response_dp_prompt3, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dp_prompt3"])
        response_dpmlm1, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dpmlm1"])
        response_dpmlm2, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dpmlm2"])
        response_dpmlm3, nodes_response_dp, evaluation_dp = getResponse(index_name, question, [database_file['file_hash'], "text_pii_dp_dpmlm3"])
...
def get_all_responses_enron():
    for i in range(11, 61): # FOR EACH DATABASE FILE (e.g (1, 6) -> "Enron_1" to "Enron_5")
        ...
        get_all_responses(table_responses_name="enron_responses2", index_name="enron", file_name=file_name, database_file=database_file)

def get_all_responses_bbc():
    for i in range(1, 11): # FOR EACH DATABASE FILE (e.g (1, 6) -> "BBC_1" to "BBC_5")
        file_name = f"BBC_{i}"
        database_file = retrieve_record_by_name("bbc_text2", file_name)
        get_all_responses(table_responses_name="bbc_responses2", index_name="bbc", file_name=file_name, database_file=database_file)
